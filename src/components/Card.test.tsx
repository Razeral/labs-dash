...
describe('Card', () => {
  it('links to the host when hosted', () => {
    render(<Card project={{ ...base, host: 'https://x.example' }} draggable={false} overridden={false} onDragStart={noop} onOpen={noop} />)
  ...
  it('marks an unhosted project unsummoned and is not a link', () => {
    render(<Card project={base} draggable={false} overridden={false} onDragStart={noop} onOpen={noop} />)
    expect(screen.getByText(/UNSUMMONED/)).toBeInTheDocument()
    expect(screen.queryByRole('link')).toBeNull()
  })

  it('never links a fallen project even with a host', () => {
    render(<Card project={{ ...base, tier: 'fallen', host: 'https://gone.example' }} draggable={false} overridden={false} onDragStart={noop} onOpen={noop} />)
    expect(screen.queryByRole('link')).toBeNull()
  })

  it('links an ascended project to its successor host', () => {
    const p: Project = { ...base, tier: 'ascended', absorbedInto: { name: 'deskboard', slug: 'ef' } }
    render(<Card project={p} successorHost="https://deskboard.example" draggable={false} overridden={false} onDragStart={noop} onOpen={noop} />)
  ...
  it('falls back to the glyph rather than printing undefined for an unnamed successor', () => {
    render(<Card project={{ ...base, tier: 'ascended' }} draggable={false} overridden={false} onDragStart={noop} onOpen={noop} />)
  ...
  it('marks an overridden card', () => {
    const { container } = render(<Card project={base} draggable={false} overridden onDragStart={noop} onOpen={noop} />)
  ...
  it('shows only the hostname in the meta line while linking to the full url', () => {
    ...
    render(<Card project={{ ...base, host }} draggable={false} overridden={false} onDragStart={noop} onOpen={noop} />)
  ...
  it('renders note when present and omits it when absent', () => {
    const { rerender } = render(<Card project={{ ...base, note: 'test note' }} draggable={false} overridden={false} onDragStart={noop} onOpen={noop} />)
    expect(screen.getByText('test note')).toBeInTheDocument()

    rerender(<Card project={base} draggable={false} overridden={false} onDragStart={noop} onOpen={noop} />)
    expect(screen.queryByText('test note')).toBeNull()
  })

  it('sets root element classes and data-slug correctly', () => {
    const { container } = render(<Card project={{ ...base, host: 'https://x.example' }} draggable={false} overridden={false} onDragStart={noop} onOpen={noop} />)
    ...
    const { container: container2 } = render(<Card project={base} draggable={false} overridden={false} onDragStart={noop} onOpen={noop} />)
  ...
  it('opens the detail view when the card body is clicked', () => {
    const onOpen = vi.fn()
    render(<Card project={{ ...base, host: 'https://x.example' }} draggable={false} overridden={false} onDragStart={noop} onOpen={onOpen} />)
  ...
  it('offers the detail view even for a card that links nowhere', () => {
    const onOpen = vi.fn()
    render(<Card project={base} draggable={false} overridden={false} onDragStart={noop} onOpen={onOpen} />)
    expect(screen.queryByRole('link')).toBeNull()
    fireEvent.click(screen.getByRole('button', { name: /details for x/i }))
    expect(onOpen).toHaveBeenCalledWith('x')
  })
